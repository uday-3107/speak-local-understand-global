...
class LiveTranscriber:
    ...
    def process(self, audio: np.ndarray, language: str | None, target: str) -> list[dict]:
        ...
        if silence < self.MIN_SILENCE_S and secs < self.FORCE_FLUSH_S:
            return []
        try:
            stt: SttResult = whisper.transcribe_np(self._buffer, language=self.language)
        except AppError:
            self._buffer = np.zeros(0, dtype=np.float32)
            return []
        ...
        translated, mt_latency_ms, mt_model = translate(phrase, self.language, self.target)
        return [
            {
                "payload": TranscriptSegmentCreate(
                    ...
                    model_used=f"{self.stt_model}->{mt_model}",
        ...
        while True:
            ...
            try:
                ...
                results = await asyncio.to_thread(
                    transcriber.process, audio, language, target
                )
                for result in results:
                    async with SessionLocal() as db:
                        segment = await session_service.create_segment(
                            db, uuid.UUID(session_id), TranscriptSegmentCreate(**result["payload"])
                        )
                    result["payload"]["id"] = str(segment.id)
                    await websocket.send_json({"type": "segment", **result})
